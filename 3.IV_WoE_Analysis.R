## Clear workspace before every run.
...
setwd('/mnt2/client16/20180501_ThickMTB/')
getwd()


         
df1 <- read.csv('/mnt2/client16/20180501_ThickMTB/2.PreparedData/thick_mtb_20180507.csv')
colnames(df1)[40]='final_remark'

vars1 <- c('N','C','Mn','Si','Cr','S','P','Ni','Cu','CE','LHT','Blower1','Blower2','Blower3','Blower4','Blower5','Blower6','UTS.Min','UTS.Max','UTS.Avg','RA','Ageing','SR.Temp','Drawing.Speed')
target_var1 <- 'final_remark'
df2=df1[,c(vars1,target_var1)]


iv = function(df=NULL,vars=NULL,unique_key=NULL,target_var=NULL,maxcat=20,numeric_cutoff=10)
{
  
  #df=df2;unique_key=NULL;target_var=target_var1;maxcat=20;vars=vars1;numeric_cutoff=5
  # unique_key="name"
  # target_var="y_flag"
  df=as.data.frame(df)
...
## Running the IV and output
# numeric_cutoff=5 so that there is not categorical variable.
iv_table=iv(df=df2,vars=vars1,target_var=target_var1,numeric_cutoff=5)
write.csv(iv_table[[3]],'/mnt2/client16/20180501_ThickMTB/IV_Analysis/iv_result1/iv_table.csv',row.names = F)
